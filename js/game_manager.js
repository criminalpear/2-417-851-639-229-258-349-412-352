    ...
    traversals.y.forEach(function (y) {
      ...
      if (tile) {
        ...
        if (next && next.value === tile.value && !next.mergedFrom) {
          ...
          // The mighty 2^81 tile
          if (merged.value === 2417851639229258349412352) self.won = true;
        } else {
...
// Console command to spawn the maximum tile (2^81)
GameManager.prototype.spawnMaxTile = function() {
  if (this.grid.cellsAvailable()) {
    var tile = new Tile(this.grid.randomAvailableCell(), 2417851639229258349412352);
    this.grid.insertTile(tile);
    this.actuate();
    console.log("Spawned 2^81 tile!");
  } else {
...
window.addEventListener('keydown', function(e) {
  // Check if F12 is pressed (developer tools)
  if (e.key === 'F12') {
    console.log("Type '2sp' in console to spawn a 2^81 tile!");
  }
});
...
// Helper function to get the closest power of 2
function getClosestPowerOf2(n) {
  if (n <= 0) return 2; // Smallest tile value
  var p = 1;
  while (p < n) {
    p <<= 1;
  }
  // Check if n is closer to p or p/2
  if (p / 2 === 0) return p; // Avoid division by zero if n is 1
  return (n - p / 2 < p - n) ? (p / 2) : p;
}

// Generic tile spawning function
GameManager.prototype.spawnTile = function(value) {
  // Ensure value is a power of 2
  var tileValue = getClosestPowerOf2(value);

  if (this.grid.cellsAvailable()) {
    var tile = new Tile(this.grid.randomAvailableCell(), tileValue);
    this.grid.insertTile(tile);
    this.actuate();
    console.log("Spawned tile with value " + tileValue + " (from input " + value + ")!");
  } else {
    console.log("No available cells to spawn tile!");
  }
};
